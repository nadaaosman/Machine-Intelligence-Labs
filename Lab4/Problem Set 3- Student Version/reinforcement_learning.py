...
class RLAgent(Agent[S, A]):
    ...
    def act(self, env: Environment[S, A], observation: S, training: bool = False) -> A:
        ...
        if training and self.should_explore():
            ...
            # NotImplemented()
            # Choose a random action if exploring during training
            # Generate a random index using the random number generator "self.rng.int"
            random_index = self.rng.int(0, len(actions) - 1)
              # Retrieve the action corresponding to the generated random index
            ...
            max_action = max(actions, key=lambda each_action: self.compute_q(env, observation, each_action))
            return max_action
...
class SARSALearningAgent(RLAgent[S, A]):
    ...
    def update(self, env: Environment[S, A], state: S, action: A, reward: float, next_state: S, next_action: Optional[A]):
        ...
        # NotImplemented()
          # If next_action is None, update Q-value for a terminal state
        if next_action is None:
            # Update Q-value using SARSA update rule for a terminal state where next_action is None
            self.Q[state][action] = self.Q[state][action]+self.learning_rate * (reward - self.Q[state][action])
        else:
              # If next_action is not None, update Q-value using SARSA update rule
...
class QLearningAgent(RLAgent[S, A]):
    ...
    def compute_utility(self, env: Environment[S, A], state: S) -> float:
        ...
        # NotImplemented()
        # Compute the utility of the given state using the maximum Q-value among all possible actions
        max_q = max(self.compute_q(env, state, action) for action in self.actions)
        return max_q
    ...
    def update(self, env: Environment[S, A], state: S, action: A, reward: float, next_state: S, done: bool):
        ...
        # NotImplemented()
        if done:
            # Update the Q-value using the Q-Learning update rule when the episode is done (next_state is a terminal state)
            # Q-value is updated based on the immediate reward and the learning rate
            self.Q[state][action] = self.Q[state][action]+self.learning_rate * (reward - self.Q[state][action])
        else:
              # If not done, update the Q-value using the Q-Learning update formula with the maximum utility of the next state
...
class ApproximateQLearningAgent(RLAgent[S, A]):
    ...
    def __compute_q_from_features(self, features: Dict[str, float], action: A) -> float:
        ...
        # NotImplemented()
        # Compute the Q-value using the linear combination of features and weights for a given action
        # The action is cast to a string before querying self.weights
    ...
    def __compute_utility_from_features(self, features: Dict[str, float]) -> float:
        # TODO: Complete this function
        # NotImplemented()
        # Calculate the utility using features by finding the maximum Q-value among all possible actions
        max_q = max(self.__compute_q_from_features(features, action) for action in self.actions)
        return max_q
    ...
    def update(self, env: Environment[S, A], state: S, action: A, reward: float, next_state: S, done: bool):
        ...
        # If done is True, then next_state is a terminal state in which case, we consider the Q-value of next_state to be 0
        # NotImplemented()
        # Calculate the utility of the next state (considered 0 for terminal states)
        utility = 0 if done else self.__compute_utility_from_features(self.feature_extractor.extract_features(env, next_state))
        # Extract features of the current state
        features = self.feature_extractor.extract_features(env, state)
        # Compute the Q-value of the current state and action
        q_value = self.compute_q(env, state, action)
        # Compute the temporal difference (difference between the actual and expected Q-values)
        difference = reward + self.discount_factor * utility - q_value
        # Update weights for each feature using the Q-Learning update rule
        # Utilizes a dictionary comprehension to update each weight    
        self.weights[action].update({feature: weight + self.learning_rate * difference * value for feature, weight, value in zip(features, self.weights[action].values(), features.values())})
